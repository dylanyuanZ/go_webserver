...
description: 性能优化技术指南，在编写代码、优化性能、进行 code review 时使用。涵盖方法论、设计原则和具体优化规则。
---

# 性能优化技术

> **示例语言说明**：本 skill 的 reference 中示例以 C++ 呈现（来自通用性能优化资料）。
> 规则本身与语言无关，落地到本项目时请转换为等价的 Go 实现
> （如 `std::vector` → slice、`std::unordered_map` → map、`std::shared_ptr` → 指针/值拷贝权衡）。
