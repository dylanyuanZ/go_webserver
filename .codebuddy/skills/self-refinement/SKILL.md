...
| **rules/skills 优先于 memory** | 沉淀经验时，**必须**写入 `.codebuddy/rules/` 或 `.codebuddy/skills/` 文件，**禁止**仅存储到 memory。memory 与个人账号绑定、不跟随项目仓库，团队成员无法共享。仅在内容纯属个人偏好（如语言、格式风格）且不涉及项目知识时才可用 memory |
