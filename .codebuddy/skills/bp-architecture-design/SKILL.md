...
description: 架构设计原则。在 system-design 阶段的 4.1 方案概览 section 被调用。提供模块划分、依赖管理、数据架构、接口设计的指导。
---
...
|----------|------|
| 按数据流 | 接入 → 校验 → 业务处理 → 持久化 |
| 按功能域 | Handler / Service / Repository / Config |
| 按技术层 | Transport / Domain / Infrastructure |
...
| 反模式 | 问题 | 改进 |
|--------|------|------|
| 循环依赖 | A→B→C→A，难以独立测试和理解 | 引入接口打破循环 |
| 边界模糊 | 多个模块都在修改同一份数据 | 明确数据所有权 |
| 过度抽象 | 不必要的接口层 | 简单优先，按需抽象 |
| 接口泄露内部 | 暴露了不该暴露的细节 | 使用专用 DTO |
| Big Ball of Mud | 无明确结构，代码到处耦合 | 逐步引入边界 |
